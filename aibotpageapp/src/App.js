...
function App() {
  ...
  const findRelevantAnswer = (query) => {
    if (!query.trim()) {
    ...
    const bestMatch = matches.reduce((prev, current) =>
      current.similarity > prev.similarity ? current : prev, { similarity: 0 });
  ...
  const handleQuerry = () => {
    if (userQuerry) {
      ...
      const answerSet = {
        ...
        answer: findRelevantAnswer(userQuerry),
        qid: newQuestionId,
      };
  ...
  const [fetchSavedHistory, setFetchSavedHistory] = useState(JSON.parse(localStorage.getItem('history')) || []);
  ...
  return (
    <div className="App">
                ...
                (<Grid size={12} sx={{ height: '80%', display: 'flex', justifyContent: 'center', alignItems: 'flex-end' }}>
                  <Stack sx={{ width: '100%' }}>
                    <Box sx={{ display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center', gap: '1em' }}>
                      <Typography variant='h3' sx={{ fontWeight: 'medium' }}>How Can I Help You Today</Typography>
                    ...
                    <Box sx={{ margin: '3em', }}>
                      <Grid container spacing={1}>
                        ...
                        <Grid size={6}
                          onClick={() =>
                            cardQuerry("Hi, What is the weather?")}
                        >
                        ...
                        <Grid size={6}
                          onClick={() =>
                            cardQuerry("Hi,What is my Location?")}
                        >
                          <Paper elevation={2} sx={{ padding: '1em', margin: '0.8em' }}
                          >
                            <Typography variant="h6" gutterBottom sx={{ fontWeight: 'bolder' }}>
                        ...
                        <Grid size={6}
                          onClick={() =>
                            cardQuerry("Hi, What is the temperature?")}
                        >
                          <Paper elevation={2} sx={{ padding: '1em', margin: '0.8em' }}>
                            <Typography variant="h6" gutterBottom sx={{ fontWeight: 'bolder' }}>
              ...
              <Grid size={12} sx={{ height: '10%' }}>
                <Grid container sx={{ height: '100%', }}>
                  ...
                  <Grid size={2}
                    ...
                    <Button variant='contained' onClick={handleQuerry}>Ask</Button>
                    <Button variant='contained' onClick={saveHistory}>Save</Button>
                  </Grid>
        ...
        {showPastConvo &&
          (
            <Grid size={10} sx={{ background: 'linear-gradient(to bottom, #fff,#eee,#D7C7F4)' }}>
              ...
              <Typography variant='h4' sx={{ textAlign: 'center', margin: '2em 0' }}>Conversation History </Typography>


              {
                fetchSavedHistory && fetchSavedHistory.length > 0 ?
                  (
                    fetchSavedHistory.reverse().map((record) => (
                      <React.Fragment id={record.id} key={record.id}>
                        <Typography variant='h5' sx={{ padding: '1em' }}>{formatDate(record.id)}</Typography>
                        ...
                        <Box
                          ...
                          {record.chathistory.map((chat) => (
                            chat.type === 'Question' ? (
                              ...
                              >
                                <Box sx={{
                                  height: '5em', width: '5em', flex: 1, borderRadius: '50%',
                                  boxShadow: '0px 8px 8px rgba(0, 0, 0, 0.2)',
                                  overflow: 'hidden'
                                }}
                                >
                              ...
                              >
                                ...
                                <Box sx={{ flex: 14 ,marginTop:'1em'}}>
                                  ...
                                  {chat.feedback ?
                                    (
                                      <Typography variant='body1' sx={{ marginTop: '1em', fontWeight: 'bolder' }}>Feedback:
                                        <Typography component='span' variant='body2' sx={{ marginLeft: '1em' }} >{chat.feedback}</Typography>
                                      </Typography>
                                    )
                                    : ''}
...

      {
        feedbackView &&
        (
          <Feedbackform feedback={feedbackData} chathistory={chatHistory} closeFeedback={setFeedbackView} setChatHistory={setChatHistory} />
        )
      }
